...
| D-36 | **スタイル名前空間の接頭辞は小文字 `spl-`**。CSSクラスは `.spl-*`、custom properties は `--spl-*`、テーマ属性は `data-spl-theme` とする。JavaScript公開API `Simplicity` は変更しない | `spl` は SimPLicity の固有略称として識別しやすく、小文字のCSS名前空間は既存のPascalCase・camelCaseクラス名と一貫して結合できる | `style_plan.md` v1.3 §7-1 |
| D-37 | **quantz-webの旧CSSクラス名台帳は、確定依存ではなく、同名非依存を含みうる保守的な依存候補として保持する** | fixture抽出前に候補を除外すると、実際の依存を見落とすため | `docs/STYLE_CLASS_INVENTORY.md` / `style_plan.md` v1.3 ST-0 |
