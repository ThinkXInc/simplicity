...
for (const shot of SHOTS) {
    ...
    // dark テーマ(ST-11): 消費側と同じ機構(Simplicity.setTheme = link 差し替え)で切り替え、
    // 独立ゴールデンとして凍結する。ギャラリーは実 UI(select)経由で切り替える。
    test(`screenshot: ${shot.name}_dark`, async ({ page }) => {
        await page.clock.install({ time: new Date('2026-07-19T00:00:00Z') });
        await page.goto(shot.url, { waitUntil: 'load' });
        await page.clock.runFor(3000);
        if (shot.name === 'gallery') {
            await page.selectOption('#gallery-theme-select', 'dark');
        } else {
            await page.evaluate(() => Simplicity.setTheme('dark'));
        }
        ...
        await expect(async () => {
            ...
            expect(themeLinks[0]).toContain('simplicity_dark.css');
        }).toPass({ timeout: 10000 });
        await expect(page).toHaveScreenshot(`${shot.name}_dark.png`, { fullPage: true });
    });
}
